...
# Load data
df <- read.csv("../results/murder_crimes_by_regions.csv", stringsAsFactors = F)
...
server <- function(input, output) {
  ...
  df %>% 
    ggvis(~population,~crimes, key := ~region) %>%
    layer_points() %>%
    add_tooltip(region_popup, "hover") %>%
    add_axis("y", title = "Количество убийств и покушений") %>%
    add_axis("x", title = "Население",title_offset = 80) %>% 
    bind_shiny("plot_1")
